...
class HtmlFormBuilder(AbstractFormBuilder):
    ...
    def add_text_field(self, field_dict):
        self.constructed_object.field_list.append('{0}:<br><input type="text" name="{1}"><br>'.format(
            field_dict['label'],
    ...
    def add_checkbox(self, checkbox_dict):
        self.constructed_object.field_list.append('<label><input type="checkbox" id="{0}" value="{1}"> {2}<br>'.format(
    ...
    def add_button(self, button_dict):
        self.constructed_object.field_list.append('<button type="button">{}</button>'.format(
            button_dict['text']
            )
        )

    def add_radio(self, radio_dict):
        self.constructed_object.field_list.append('<input type="radio" name="{0}" id="{1}" value="{2}"><label for="{1}">{3}</label><br><input type="radio" name="{0}" id="{4}" value="{5}"><label for="{4}">{6}</label><br>'.format(
            radio_dict['name'],
            radio_dict['field_id_1'],
            radio_dict['value_1'],
            radio_dict['label_1'],
            radio_dict['field_id_2'],
            radio_dict['value_2'],
            radio_dict['label_2']
            )
        )
...
if __name__ == "__main__":
    ...
    field_list = [
        ...
        },
        {
            "field_type": "radio",
            "name": "check_those",
            "field_id_1": "check_this_one",
            "field_id_2": "check_this_two",
            "value_1": "1",
            "value_2": "2",
            "label_1": "Check for one",
            "label_2": "Check for two",
        },
        {
            "field_type": "button",
            "text": "DONE"
    ...
    with open("form_file.html", 'w') as f:
        f.write("<html><body>{0!r}</body></html>".format(director.get_constructed_object()))
